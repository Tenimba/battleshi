...
(function (global) {
    ...
    var game = {
        ...
        init: function () {
            ...
            this.battleship_img = document.querySelector(".fleet .battleship");
            this.destroyer_img = document.querySelector(".fleet .destroyer");
            this.submarine_img = document.querySelector(".fleet .submarine");
        this.small_ship_img = document.querySelector(".fleet .small-ship");
  ...
  utils.info("Placez vos bateaux");
        ...
        addListeners: function () {
            // on ajoute des écouteur uniquement sur la grid (délégation d'événement)
        ...
        handleClick: function (e) {
            ...
            if (e.target.classList.contains("cell")) {
                ...
                if (this.getPhase() === this.PHASE_INIT_PLAYER) {
                    ...
                    if (this.players[0].setActiveShipPosition(this.col = utils.eq(e.target), this.row = utils.eq(e.target.parentNode))) {
                    ...
                    // si l'emplacement du bateau est deja occuper on recommence 
                // si on est dans la phase de jeu (du joueur humain)
                } else if (this.getPhase() === this.PHASE_PLAY_PLAYER) {
                    // APPEL DE LA METHODE play DANS LE QUEL EST APPLER LA METHODE fire
        ...
        fire: function (from, col, line, callback) {
            ...
            var submarine_HP = [];
            var small_ship_HP = [];
            var grid = self.players[1].grid;   


            // determine qui est l'attaquant et qui est attaqué
            var P = this.players.indexOf(from) === 0
            ? this.players[1]
            : this.players[0];
            ...
            P.receiveAttack(col, line, function (hasSucceed) {
                if (hasSucceed) {
                    msg += "Touché !";
                    const audio = new Audio("audio/reussi.mp3");
                    ...
                    msg += "Manqué...";
                    const audio = new Audio("audio/echec.mp3");
                    audio.play();
                    self.renderMap(true);

                };

                grid.forEach(x => {  if(x.includes(5)) {battleship_HP.push(x)  } });
                grid.forEach(x => { if(x.includes(6)) { destroyer_HP.push(x) } });
                grid.forEach(x => { if(x.includes(7)) { submarine_HP.push(x) } });
                grid.forEach(x => { if(x.includes(8)) { small_ship_HP.push(x) } });

                if(battleship_HP.length === 0  && self.battleship_img.classList.contains('sunk') === false){
                    self.battleship_img.classList.add('sunk');
                }
                if(destroyer_HP.length === 0 && self.destroyer_img.classList.contains('sunk') === false){
                    self.destroyer_img.classList.add('sunk');
                }
                if(submarine_HP.length === 0  && self.submarine_img.classList.contains('sunk') === false){
                    self.submarine_img.classList.add('sunk');
                }
                if(small_ship_HP.length === 0  && self.small_ship_img.classList.contains('sunk') === false){
                    self.small_ship_img.classList.add('sunk');
                }
